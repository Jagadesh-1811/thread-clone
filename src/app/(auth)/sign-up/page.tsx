...
export default function SignUpPage() {
  ...
  return (
    ...
    >
      <Card className="glass border-white/10 text-white shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-px bg-linear-to-r from-transparent via-white/20 to-transparent" />

        <CardHeader className="space-y-1 text-center pb-4 border-b border-white/5">
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4 pt-6">
            {error && (
              <motion.div 
                ...
                className="text-red-400 text-sm text-center py-3 rounded-xl bg-red-500/10 border border-red-500/20 font-medium"
              >
                {error}
              </motion.div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <motion.div variants={itemVariants} className="space-y-2">
                <Label htmlFor="name" className="text-neutral-300 ml-1 font-medium">Full Name</Label>
                <Input 
                  id="name" 
                  name="name" 
                  required 
              ...
              <motion.div variants={itemVariants} className="space-y-2">
                <Label htmlFor="username" className="text-neutral-300 ml-1 font-medium">Username</Label>
                <Input 
                  id="username" 
                  name="username" 
                  required 
            ...
            <motion.div variants={itemVariants} className="space-y-2">
              <Label htmlFor="password" className="text-neutral-300 ml-1 font-medium">Password</Label>
              <div className="relative">
                <Input 
                  ...
                  required 
                  className="bg-white/5 border-white/10 text-white h-12 pr-12 focus:ring-2 focus:ring-white/20 focus:border-white/30 transition-all rounded-xl shadow-inner shadow-black/20" 
                />
                <button 
                  type="button"
          ...
          <CardFooter className="flex flex-col space-y-4 pb-8">
            ...
            <motion.div variants={itemVariants} className="text-sm text-center text-neutral-400">
              ...
              <Link href="/sign-in" className="text-white hover:underline font-semibold decoration-white/30 underline-offset-4">
                Sign in
              </Link>
            </motion.div>
          </CardFooter>
